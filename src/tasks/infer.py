...
class infer_from_trained(object):
    def __init__(self, args=None, detect_entities=False):
        ...
        if self.args.model_no == 0:
            ...
            lower_case = False
            model_name = 'BioBERT'
            config = BertConfig.from_pretrained(os.getcwd() + '/additional_models/biobert_v1.1_pubmed/bert_config.json')
            self.net = BertModel.from_pretrained(pretrained_model_name_or_path=os.getcwd() + '/additional_models/biobert_v1.1_pubmed/biobert_v1.1_pubmed.bin', 
                                                 config=config,
    ...
    def infer_sentence(self, sentence, detect_entities=False):
        if detect_entities:
            ...
            if sentences != None:
                ...
                pred = self.infer_one_sentence(sentences)
                preds.append(self.rm.idx2rel[pred].strip())#pred)
                e1 = sentences[sentences.find("[E1]") + 4 : sentences.find("[/E1]")]
                e2 = sentences[sentences.find("[E2]") + 4 : sentences.find("[/E2]")]
                return (e1, e2, preds)
        else:
...
class FewRel(object):
    def __init__(self, args=None):
        ...
        if os.path.isfile(os.getcwd() + '/src/data/%s_tokenizer.pkl' % model_name):
            self.tokenizer = load_pickle("%s_tokenizer.pkl" % model_name)
            logger.info("Loaded tokenizer from saved file.")
        else:
            logger.info("Saved tokenizer not found, initializing new tokenizer...")
            if args.model_no == 2:
                self.tokenizer = Tokenizer(vocab_file=os.getcwd() + '/additional_models/biobert_v1.1_pubmed/vocab.txt',
                                           do_lower_case=False)
            else:
        ...
        if self.args.use_pretrained_blanks == 1:
            ...
            checkpoint_path = os.getcwd() + "/src/data/test_checkpoint_%d.pth.tar" % self.args.model_no
